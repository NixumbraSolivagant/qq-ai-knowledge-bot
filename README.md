...
- `SOURCE_DOCUMENT_LIMIT`：每轮探测的教材/课程/官方教程数量，默认 2 个
- `SOURCE_FEED_LIMIT`：每轮探测的技术博客和项目 Feed 数量，默认 2 个
- `LOCAL_KB_ENABLED`：优先使用本机教材知识库，默认开启

资料抓取是软依赖：某个网站不可访问时会自动跳过；所有来源都不可用、正文不完整或摘要未通过校验时，本次不会发送未经证实的知识内容。每日选题会在 Python、数据结构、机器学习、线性代数、深度学习和大语言模型等本科生方向间切换。

`今日知识` 会优先选择教材、课程讲义或官方学习网站，再把一篇资料改造成“今天学什么、最小示例、练习题、答案提示”的本科生微课；先生成凝练稿，再进行一次逐句原文对照审校。程序还会校验数字、证据编号和来源网址，并自动附上两条原文摘录；未通过校验时不会发送未经证实的摘要。

运行 `python scripts/build_knowledge_base.py` 可下载并提取本地教材。教材 PDF 和索引位于 `data/knowledge_base/`，已被 Git 忽略，不会上传公开仓库；机器人会按片段顺序学习，避免每天重复随机摘要。
